...
# Version: transformers==4.53.2; trl==0.19.1
...
from utils.data_sotopia import ms_prompt
import tqdm
...
class Trainer:
    ...
    def __init__(
        ...
        judge: Union[PreTrainedModel, nn.Module, None] = None,
        judge_tokenizer: Optional[
        ...
        ## Load evaluation simultation instances (randomly sampled from trainingset)
        sample_filepath = "data/gemini-tom-conversation-sotopia-pi-episodes.json"
        self.eval_best_score = -1
        self.early_stop = 0
        ...
        if os.path.exists(sample_filepath):
            eval_simulation_data = json.load(open(sample_filepath, "r", encoding="utf-8"))
            self.eval_simulation_data = random.sample(eval_simulation_data, 20)
        else:
            self.eval_simulation_data = []
        logger.info(f"[({len(self.eval_simulation_data)}) eval simulation data loaded!]")
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            ## stop early if the best model hasn't been updated more than 3 times
            if self.early_stop > self.early_stop_count:
                break            
    ...
    def _evaluate_simulation(self, model):
        # print(len(self.eval_simulation_data))
        model.eval()
        if not self.eval_simulation_data:
            return None
        ...
        @torch.no_grad()
        def _sample(model, chat, do_sample=True, temperature=0.7, max_new=200, top_p=0.9, tokenizer=None):
            if tokenizer is not None:
                inputs = tokenizer.apply_chat_template(chat, return_tensors="pt", tokenize=True, return_dict=True, add_generation_prompt=True).to(model.device)
            else:
                inputs = self.processing_class.apply_chat_template(chat, return_tensors="pt", tokenize=True, return_dict=True, add_generation_prompt=True).to(model.device)
            output = model.generate(**inputs, do_sample=do_sample, temperature=temperature, top_p=top_p, max_new_tokens=max_new)
            if tokenizer is not None:
                decoded = tokenizer.decode(output[0, inputs['input_ids'].shape[-1]:], skip_special_tokens=True)
            else:
                decoded = self.processing_class.decode(output[0, inputs['input_ids'].shape[-1]:], skip_special_tokens=True)
            return decoded
        ...
        def _evaluate_goal_achievement(scenario, social_goals, speaker_a, speaker_b, history):
            scores = {}
            for speaker in [speaker_a, speaker_b]:
                chat = [{
                    "role": "user",
                    "content": GOAL_EVAL_PROMPT.format(
                        scenario=scenario, agent=speaker,
                        social_goal=social_goals[speaker],
                        history="\n".join(history)
                    )
                }]
                # response = self.call_genimi(prompt, max_gen_len=500, temperature=0.0)
                response = _sample(model=self.judge, chat=chat, do_sample=True, temperature=0.7, max_new=500, top_p=0.9, tokenizer=self.judge_tokenizer)
                score = _parse_score(response)
                scores[speaker] = {
                    "score": score if score != -1 else 5,
                    "reasoning": response
                }
            avg_score = (scores[speaker_a]["score"] + scores[speaker_b]["score"]) / 2
            return avg_score
        
        all_scores = []
        for item in tqdm.tqdm(self.eval_simulation_data):
            scenario = item["scenario"]
            social_goals = item["social_goals"]
            speaker_a = item["agent1_name"]
            speaker_b = item["agent2_name"]
            # agents_background = item["agents_background"]
            history = []
            is_conversation_end = False
            is_error = False
            for _ in range(10):
                try:
                    for model, speaker in [(model, speaker_a), (model, speaker_b)]:
                        if speaker == speaker_b:
                            chat = [{
                                "role": "user",
                                "content": prompt_utterance_without_ms(
                                    history, speaker, scenario, social_goals[speaker], turn_number=len(history)+1,
                                )
                            }]
                            utterance = _sample(model=model, chat=chat, do_sample=True, temperature=0.7, max_new=200, top_p=0.9)
                            parsed = parse_action(utterance)
                            utterance = construct_utterance(parsed, speaker)
                            history.append(utterance)
                        else:
                            if self.train_type == "ms" or self.train_type == "ms-uttr-sep":
                                chat = [{
                                    "role": "user",
                                    "content": ms_prompt(
                                        history="\n".join(history) if history else "(no history — conversation not started)",
                                        depth=1,
                                        person=speaker_a,
                                ...
                                ms = _sample(model=model, chat=chat, do_sample=True, temperature=0.7, max_new=400, top_p=0.9)
                                chat = [{
                                    "role": "user",
                                    "content": prompt_utterance_with_ms(
                                        history, speaker, ms, scenario, social_goals[speaker], turn_number=len(history)+1,
                                    )
                                }]
                                utterance = _sample(model=model, chat=chat, do_sample=True, temperature=0.7, max_new=200, top_p=0.9)
                                parsed = parse_action(utterance)
                                ...
                                raise ValueError(f">>> ValueError: unsupported `train_type`: {self.train_type}")
                    ...
                    break
            if not is_error:
                avg_score = _evaluate_goal_achievement(scenario, social_goals, speaker_a, speaker_b, history)
                logger.info(f">>> _evaluate_simulation: [avg_score = {avg_score}]")
                all_scores.append(avg_score)
            else:
                logger.info(">>> error exists.. skip calculating avg. score")
        model.train()
        final_avg_score = sum(all_scores)/len(all_scores) if all_scores else 0.0
        self.log({"eval_score": final_avg_score})
    ...
    def _maybe_log_save_evaluate(
        ...
        if self.control.should_save:
            eval_score = self._evaluate_simulation(model)
            if eval_score > self.eval_best_score:
                logger.info(f"Update the checkpoint! current score: {eval_score}, the previous best score: {self.eval_best_score} !!")
                self.eval_best_score = eval_score
                self.early_stop = 0 # initialize early stop if there was an update
                self.log({"best_score": self.eval_best_score})
                self._save_checkpoint(model, trial)
                self.control = self.callback_handler.on_save(self.args, self.state, self.control)
            else:
                self.early_stop += 1
                logger.info(f"Skip saving..current score: {eval_score} and previous best score: {self.eval_best_score}")
